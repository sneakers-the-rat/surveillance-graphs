...
\emph{edges} that describe their relationship\sidenote{Equivalently, one
...
access to data by creating captive data subjects\sidenote{Facebook
...
In contrast, merging graphs is more straightforward\sidenote{\begin{quote}
...
The NIH's Biomedical Data Translator\sidenote{Or, just ``Translator''}
...
impossible\sidenote[][-2in]{"First, we assert that a single monolithic data set that directly
  ...
  shifting and exponentially growing. [...] We also assert that
  ...
  technologies that produce them, as well as the nuances of language."\cite{consortiumUniversalBiomedicalData2019} 
  }\cite{consortiumUniversalBiomedicalData2019} . The
...
rather than inaccuracy or inequity\sidenote[][-0.75in]{In an odd mixture of
  ...
  Babel.'' \cite{austinDeconstructingTranslationalTower2019}  It is unclear why an effort to create a universalizing ontology
  would be deconstructing a tower of babel, as it was the power of
  a unified language that allowed it to be built.}.
...
``Slots\sidenote{or links, labeled edges.},'' like a therapeutic
procedure that
...
than accomodating ``raw data\sidenote{In a 2018 presentation by one of
...
\textbf{always} improves the computations performed on it\sidenote{The
...
\item
  ...
  describe their entrance into the patient data market\sidenote{\begin{quote}
...
society\sidenote{If you could rig an MS Word template to punctuate
  sentences with
...
Graph of Everything. They check all the boxes\sidenote{Hopefully this
...
prosecution, and the rest. A \textbf{Decarceration} group\sidenote{Including
...
used for\sidenote{Palantir prides itself on its ability to continuously
  add new data sources: \textgreater{} ``Because one of Palantir's
  ...
  databases feeding Palantir at JRIC.'' \cite{braynePredictSurveilData2020} }.}
...
``Homelessness OKN'' group, takes particular effort\sidenote{The
  ...
  counterincentive to series ethical consideration of the projects ---
  and also a demonstration of why it might not be good to organizing
...
resources''\sidenote{\ldots and then outsourcing the maintenance and
...
in turn causing them to measure or create more crime\sidenote{\begin{quote}
...
This is the properly \emph{managerial} core of the positive vision of
platform-driven mass surveillance: that a grand, unified graph of
everything will allow the truth to emerge from the Big Data so that
decisionmakers can divine what is best for the commoners who could not
possibly understand the complexities of their health, environment, or
social systems themselves. The conflict with the reality of the
bureaucracies that might enact or support them temper those dreams, to
the degree the prevailing corporate design logics of cloud-based
infrastructure and what constitutes a fundable idea hadn't
already\sidenote{For a fuller discussion of utopias, power, imagination,
  managerialism, and its intersections with bureaucracy, see \cite{graeberUtopiaRulesTechnology2015}  \textgreater{} The increasing
  ...
  catastrophic. {[}\ldots{]} \textgreater{} \textgreater{} A timid,
  ...
  implications. {[}\ldots{]} \textgreater{} \textgreater{} This is what
  ...
  \cite{graeberUtopiaRulesTechnology2015} }. Rather than a
smoothly flowing ocean of data, their fate is more likely to extend and
elaborate the disjointed wash of
\href{https://en.wikipedia.org/wiki/Software_as_a_service}{SaaS} that
defines institutional information systems. The petty tyrants will will
gain new widgets on their dashboards rented from the information
companies who absorbed the ontologies and harmonized data sources,
automating their discretion under the guise of objectively meausurable
fact. Meanwhile the new housing capacity might never arrive.
...
researchers or the public at large. These and other\sidenote{It's out of
...
That these tradeoffs are either not considered or seen as the natural
...
claim I had a Complete and True Perspective\sidenote{Universal
...
``more ethics in AI,'' but that The Cloud\sidenote{In addition to
  already thoroughly problematized ideologies like immortality cults and
  their many branches like longtermism, etc.} poses its own strong and
...
everything\sidenote{(that is profitable to maintain IP licenses for)}
...
Services are \emph{rented} or given away freely\sidenote{The notion of
  ...
  described below: \textgreater{} ``In this, time-sharing anticipated
  ...
  stolen time. {[}\ldots{]} \textgreater{} \textgreater{} Virtualization
  ...
  etc., as we personally want).\cite{huPrehistoryCloud2015} }
...
\textbf{The Platform has agency.} Computational ``agents'' are
...
Users\sidenote{Literally ``Harnessing the wisdom of the crowds'' \cite{pascaOrganizingSearchingWorld2007} }} --- without the Platform
...
something that users consume. Information is \textbf{true or false}
regardless of context or positionality. \textbf{Ambiguity is a bug} -
there is a single True way of describing the world\sidenote{Google
  ...
  evaluators: \textgreater{} ``Ratings should not be based on your
  ...
  your rating locale.'' \cite{googleSearchQualityEvaluator2022} }.
...
information in its context. \textbf{More data is better\sidenote{See
  Data Feminism's concept of ``Big Dick Data'' \textgreater{} Big Dick
  ...
  \cite{dignazioDataFeminism2020} }.} Uncertainty is a deviation
from some underlying, natural, true value, and can be fixed by having
more or higher \emph{quality} data \cite{halevyUnreasonableEffectivenessData2009} . Where users make content,
...
platform refines data into Knowledge\sidenote{\begin{quote}
  ...
  \end{quote}}. \textbf{The Platform knows better} than individual,
...
not a bug\sidenote{
  ...
  the user's ongoing task and intentions.\\
  ...
  them. The boundary between the SERP and target pages is fluid'' \cite{kallinikosAmbivalentOntologyDigital2013}}. The cognitive expectation of indexical or ``advanced''
...
data. The authors, again correctly, point to the dangers of overhyping
...
others\sidenote{eg. from ``Resisting AI:'' \textgreater{} What's
  ...
  produce the world. \cite{mcquillanResistingAIAntifascist2022} }
...
\textbf{personal assistants} like Apple's Siri\sidenote{Interestingly
  ...
  \textgreater{} ``Siri's various teams morphed into an unwieldy
  ...
  ``Samantha,'' a conversant operating system.'' \cite{tilleySevenYearItchHow2018} }, Amazon Alexa, and Google Home.
...
range of possibilities, and flattens the act of sensemaking to a single,
ostensibly ``true'' answer \cite{shahSituatingSearch2022} . This
is, again, true\sidenote{Though Google specifically is very aware of
  multiple search strategies and address the need to better accomodate
  them elsewhere.}, but also the goal. The Cloud Orthodoxy specifically
privileges search strategies that minimize cognitive burden, imagining
Users as busy executives and the role of platform to guide them on a
``search journey.'' The transformation of the search bar into the
\emph{prompt} is intended to capture more of the ``burden'' of search
inside the platform --- the notoriously difficult problem of parsing
ambiguous subjects like the ``jaguar'' example above can be resolved by
identifying multiple candidate entries in a knowledge graph and simply
asking the user which one they meant\sidenote{Again, invoking
  convenience: \textgreater{} It has been a powerful vision for more
  ...
  \cite{adolphsBoostingSearchEngines2022} } \cite{bharadwajDependencyGraphGeneration2020} .

The lens of search recenters our focus away from the \emph{generative}
...
of the Semantic Web\sidenote{\begin{quote}
  ...
  \end{quote}}\sidenote{The question ``Where are the agents?'' was
...
integration\sidenote{A common pattern described by platform studies
  literature, eg. see \cite{blankeMaterialConditionsPlatforms2020}  and their description of the role of extended and tangled
  software ecosystems in the maintenance of platform dominance.}.
...
Historically, these personal assistants have worked badly
...
actively make recommendations without an explicit query \sidenote{One
  ...
  \textgreater{} ``The zero-query search paradigm can be expressed with
  the slogan ``the query is the user.'' In practice, the context of the
  user is used to infer information needs.'' \cite{balogEntityOrientedSearch2018} }. Google demonstrates being able to
...
personalize recommendations\sidenote{\begin{quote}
  ...
  \end{quote}} and systems for being able to ``target campaigns to users
...
%good user. I have been a good chatbot'' \cite{curious\_evolverCustomerServiceNew2023} . An example in the GPT-4
...
whether it ``works\sidenote{Arguably, the fact that ever more
  surveillance data needs to be gathered is a sign that ads \emph{don't}
  work all that well, and some have made the case that targeted
  advertising is a bubble \cite{edelmanAdTechCould2020} },'' but
...
\includegraphics[width=\linewidth]{img/rethinking_search_f1-01.png} \emph{Recreation of
...
hallucinate statistical patterns\sidenote{``While large language models
  are brilliantly creative, they're also fallibile. That's why grounding
  the LLM in data is so important.'' \cite{microsoftFutureWorkAI2023} }. They are invoked as a complementary strategy with deep-learning
...
fundamental lack:\sidenote{\begin{quote}
  ...
  \end{quote}} as a service, some functionality must always be withheld
...
guess?\sidenote{Before search engines were seen as an invisible,
  ...
  Cataloging'' \cite{jjournalofinternetcatalogingJournalInternetCataloging1999} , and
  ...
  strategy: \textgreater{} I think it all boils down to whether we want
  ...
  result simply means more information is needed. \textgreater{}
  \textgreater{} I myself have always presumed that SW agents would
  ...
  prefer that they wouldn't be guessing.} \cite{sticklerReMonotonicityWas2002} }'' Why do we have to work around
...
we \emph{can} build a better world\sidenote{\begin{quote}
  ...
  \end{quote}}, to list all the many ways we are hemmed in by
...
action\sidenote{In the words of CrimethInc: I am not giving
  instructions, but license. \textgreater{} Above all! It means not
  accepting this or any manifesto or definition as it is, but making and
  remaking it for yourself. \cite{crimethincex-workerscollectiveFightingOurLives2002} }. Our old
...
the loosely-defined ``fediverse\sidenote{I give a fuller description of
...
fork\sidenote{``Forking'' in digital social spaces is different than in
  physical spaces, where resources can be duplicated and split \cite{MeatballWikiRightToFork} } code and ourselves freely \cite{bookchinNoteAffinityGroups1969, MeatballWikiRightToFork} .
